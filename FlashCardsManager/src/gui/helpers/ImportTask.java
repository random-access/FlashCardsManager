...
public class ImportTask extends SwingWorker<Void, Void> implements
		IProgressPresenter {
	String pathToImport;
	...
	public ImportTask(String pathToImport, ProgressDialog dialog,
			MainWindow mw, ProjectsController ctl) {
	...
	protected Void doInBackground() {
		...
		try {
			if (FileUtils
					.directoryContainsCertainFines(
							pathToImport,
							new ArrayList(
									ArrayUtil
											.asReadOnlyList(new String[] { "collection.anki2" })))) {
			...
			if (FileUtils.directoryContainsCertainFines(
					pathToImport,
					new ArrayList(ArrayUtil.asReadOnlyList(new String[] {
							"flashcards.xml", "media.xml", "projects.xml" })))) {
